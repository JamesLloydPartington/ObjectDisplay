...
class Translation
{
  public:
    ...
    void Rotate(double ThetaX, double ThetaY, double ThetaZ)
    {
      ...
      for(i = 0; i < NPoints; i++)
      {
        ...
        for(j = 0; j < 3; j++)
        {
          for(k = 0; k < 3; k++)
          {
            Temp.Corner[j]+= R[j][k] * AllCorner[i].Corner[k];
          }
        }

        for(j = 0; j < 3; j++)
        {
          AllCorner[i].Corner[j] = Temp.Corner[j];
        }
      }
    }
    ...
    void Shift(double X, double Y, double Z)
    {
      ...
      for(i = 0; i < NPoints; i++)
      {
        AllCorner[i].Corner[0]+=X;
        AllCorner[i].Corner[1]+=Y;
        AllCorner[i].Corner[2]+=Z;
      }
    }
...
{
  public:
    Corners* CubeCorner = (Corners*)calloc(8, sizeof(Corners));
    Edges* CubeEdge = (Edges*)calloc(12, sizeof(Edges));
    Surfaces* CubeSurface = (Surfaces*)calloc(6, sizeof(Surfaces));

    Corners CubeCenter;

    void SetCenter(Corners Center)
    {
      CubeCenter = Center;
    }
    ...
    Cubes(Corners Center, double size)
    {
      ...
      for(i = 0; i < 2; i++)
      {
        ...
        for(j = 0; j < 2; j++)
        {
          ...
          for(k = 0; k < 2; k++)
          {
            z = ((2 * k) - 1) * size / 2;
            Corners Corner;
            CubeCorner[i * 2 * 2 + j * 2 + k] = Corner;
            CubeCorner[i * 2 * 2 + j * 2 + k].MakeCorners(x, y, z);
          }
        }
      }
      ...
      for(i = 0; i < NCubeEdge; i++)
      {
        ...
        for(j = 0; j < NCorner; j++)
        {
          CubeEdge[i].Edge[j] = CubeCorner[EdgeToCorner[i][j]];
        }
      }

      for(i = 0; i < NCubeSurface; i++)
      {
        Surfaces Surface(NEdge);
        CubeSurface[i] = Surface;

        for(j = 0; j < NEdge; j++)
        {
          CubeSurface[i].Surface[j] = CubeEdge[FaceToEdge[i][j]];
        }
      }
      AllCorner = CubeCorner;
    }
};
...
void draw_Points(Corners* AllCorners, Corners Center, int NCorners, coordInfo Grid)
{
  ...
  {
    i = Grid.xPix(AllCorners[l].Corner[0] + Center.Corner[0]);
    j = Grid.yPix(AllCorners[l].Corner[1] + Center.Corner[1]);
    k = Grid.yPix(AllCorners[l].Corner[2] + Center.Corner[2]);
...
void events_SDL()
{
  ...
  while (!close) //While SDL window is open
  {
    ...
    MyCube.Rotate(sin(phiX) / 300, sin(phiY) / 300, sin(phiZ) / 300);

    MyCubeCenter.MakeCorners(sin(x), cos(y), sin(z));
    MyCube.SetCenter(MyCubeCenter);
    draw_Points(MyCube.CubeCorner, MyCube.CubeCenter, 8, Grid);
